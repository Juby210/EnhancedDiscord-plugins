window.JubyLib = {
    load() { 
        var css = `#JubyLib-container h2.ui-form-title {
            font-size: 16px;
            font-weight: 600;
            line-height: 20px;
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if(pluginVersion <= v && pluginVersion != v) {
                window.JubyLib.popup("Plugin update", `${pluginName} has an update!<br>Click download button to download<br><br>Your Version: ${pluginVersion}<br>New Version: ${v}`, "Download", "10px", () => require("electron").shell.openExternal(pluginUrl));
            }
        }
    });
};
