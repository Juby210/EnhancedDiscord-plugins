// this "lib" is DEPRECATED, please don't use this :v
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    if(plugin.version <= v && plugin.version != v) {
                        module.exports.notice(`Plugin update: <strong>${plugin.name}</strong> (${plugin.version} => ${v})`, "Download", "Premium", () => {
                            const request = require("request");
                            const fs = require("fs");
                            const path = require("path");
                            request(pluginUrl, (err, res, body) => {
                                if(err) return module.exports.notice(`Failed to download <strong>${plugin.name} ${v}</strong>`, undefined, "Danger");
                                let fn = plugin.id+'.js';
                                let file = path.join(module.exports.getPluginsDir(), fn);
                                fs.writeFileSync(file, body);
                                module.exports.notice(`Updated <strong>${plugin.name}</strong> from version ${plugin.version} to ${v}`, "Reload", "Spotify", () => {
